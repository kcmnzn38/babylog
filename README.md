...
- **絵本マスタ**: 本の裏のバーコード(ISBN)をカメラで読み取ると、書名・作者・表紙を
  自動取得（openBD → Google Books・無料/キー不要）してマスタに登録。
  「えほん」の記録でマスタから選ぶと、どの絵本を何回読んだかが残ります（家族で同期）
...
写真の表示は「合言葉を確認 → 期限つきの署名付きURLへ302リダイレクト」方式で、
画像本体はCDNから直接配信されます（サーバー関数を経由しないため無料枠の転送量を
ほぼ消費せず、端末側にも長期キャッシュされます）。
